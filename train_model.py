...
class load_data(tf.keras.utils.Sequence):
    ...
    def __getitem__(self, index):
        ...
        for filename in batch:
            ...
            # Load mask
            mask = cv2.imread(os.path.join("dataset", \
                "masks", "cutter", filename + ".png"), \
                cv2.IMREAD_GRAYSCALE)
            mask_bg = cv2.bitwise_not(mask)

            mask = np.dstack((mask,mask_bg))
        ...
        # print("X data = ",batch_x.shape)
        # print("Y data = ",batch_y.shape)
...
def create_callback():
    ...
    def predict_epoch(epoch, logs):
        ...
        class_cutter = np.concatenate([image, mask[:, :, 0], image * mask[:, :, 0]], axis = 1)
        class_bg = np.concatenate([image, mask[:, :, 1], image * mask[:, :, 1]], axis = 1)

        # Log the image as an image summary.
        with file_writer.as_default():
            tf.summary.image("class_cutter", np.reshape(class_cutter, (1, image_size[0], image_size[1]*3, 3)), step=epoch)
            tf.summary.image("class_bg", np.reshape(class_bg, (1, image_size[0], image_size[1]*3, 3)), step=epoch)
...
train_ds = load_data(train_list, \
    batch_size=batch_size, image_size=image_size, \
    augmentation=aug)

print("Load Test Dataset")
test_ds = load_data(test_list, \
    batch_size=batch_size, image_size=image_size)
